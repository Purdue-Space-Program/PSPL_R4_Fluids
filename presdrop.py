...
def rounded_entrance_dp_eth(r_c, d_outer, thic, ar):
    (d_inner, line_vel_eth, line_vel_ox, re_eth, re_ox, fric_eth, fric_ox, rel_rough) = pipe_properties(d_outer, thic, ar)
    d_inner = d_inner.to(u.meter)
    r_c = r_c.to(u.meter)
    K = fittings.entrance_rounded(Di = d_inner, rc = r_c, method = 'Crane')
    dp = core.dP_from_K(K, rho=rho_eth.to(u.kilogram / (u.meter**3)).magnitude, V=line_vel_eth.to(u.meter / u.second).magnitude) * u.pascal
    return dp, K

def rounded_entrance_dp_ox(rad_c, d_outer, thic, ar):
    (d_inner, line_vel_eth, line_vel_ox, re_eth, re_ox, fric_eth, fric_ox, rel_rough) = pipe_properties(d_outer, thic, ar)
    d_inner = d_inner.to(u.meter)
    r_c = r_c.to(u.meter)
    K = fittings.entrance_rounded(Di = d_inner, rc = r_c, method = 'Crane')
...
def venturi_dp_eth(K, d_outer, thic, ar):
    (d_inner, line_vel_eth, line_vel_ox, re_eth, re_ox, fric_eth, fric_ox, rel_rough) = pipe_properties(d_outer, thic, ar)
    # This calculation will take the pressure before the venturi, then calculate venturi drop, and then use this to calculate total drop
    dp_inlet = core.dP_from_K(K, rho=rho_ox.to(u.kilogram / (u.meter**3)).magnitude, V=line_vel_ox.to(u.meter / u.second).magnitude) * u.pascal
    current_ox = dp_inlet + injector_dp.to('Pa')
    dp_total = current_ox.to(u.psi) - dp_inlet.to(u.psi)
    dp_total += 0.2 * current_ox.to(u.psi)
    return dp_total, current_ox


def venturi_dp_ox(d_outer, thic, ar):
    (d_inner, line_vel_eth, line_vel_ox, re_eth, re_ox, fric_eth, fric_ox, rel_rough) = pipe_properties(d_outer, thic, ar)

def pump_dp_eth(d_outer, thic, ar):
    (d_inner, line_vel_eth, line_vel_ox, re_eth, re_ox, fric_eth, fric_ox, rel_rough) = pipe_properties(d_outer, thic, ar)
    # Since we are given an inlet and outlet pressure for the pump, this can be hard coded into the dp algorithm along with accounting the bend geometry

def pump_dp_ox(d_outer, thic, ar):
    (d_inner, line_vel_eth, line_vel_ox, re_eth, re_ox, fric_eth, fric_ox, rel_rough) = pipe_properties(d_outer, thic, ar)
    # Since we are given an inlet and outlet pressure for the pump, this can be hard coded into the dp algorithm along with accounting the bend geometry
...
def sharp_expansion_dp_eth(d_outer_1, d_outer_2, thic, ar):
    ...
    K = fittings.diffuser_sharp(d_inner_1, d_inner_2, re_eth, fric_eth, rel_rough, 'Rennels')
    dp = core.dP_from_K(K, rho=rho_eth.to(u.kilogram / (u.meter**3)).magnitude, V=line_vel_eth.to(u.meter / u.second).magnitude) * u.pascal
    dp = dp.to(u.psi)
    return dp, K
...
def sharp_expansion_dp_ox(d_outer_1, d_outer_2, thic, ar):
    ...
    K = fittings.diffuser_sharp(d_inner_1, d_inner_2, re_ox, fric_ox, rel_rough, 'Rennels')
    dp = core.dP_from_K(K, rho=rho_ox.to(u.kilogram / (u.meter**3)).magnitude, V=line_vel_ox.to(u.meter / u.second).magnitude) * u.pascal
    dp = dp.to(u.psi)
    return dp, K
...
def main():
    ...
    dp_ox_straight, k_ox_straight = straight_dp_ox(length1, d_outer_tube, tube_thic, abs_rough)
    print(f"Pressure drop for LOx in straight tube: {dp_ox_straight}")
    dp_ox_bend, k_ox_bend = bend_dp_ox(angle1, rad1, d_outer_tube, tube_thic, abs_rough)
    print(f"Pressure drop for a bend (LOx): {dp_ox_bend}")
    dp_ox_bv, k_ox_bv = ball_valve_dp_ox(cv_bv, d_outer_tube, tube_thic, abs_rough)
    print(f"Pressure drop across ball valve (LOx): {dp_ox_bv}")
    dp_ox_cont, k_ox_cont = sharp_contraction_dp_ox(d_outer_tube, d_outer_tube * 0.5, tube_thic, abs_rough)
    print(f"Pressure drop across sharp contraction (LOx): {dp_ox_cont}")
    dp_ox_exp, k_ox_exp = sharp_expansion_dp_ox(d_outer_tube, d_outer_tube * 2, tube_thic, abs_rough)
    print(f"Pressure drop across sharp expansion (LOx): {dp_ox_exp}")
